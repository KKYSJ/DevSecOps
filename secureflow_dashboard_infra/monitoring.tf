...
resource "aws_cloudwatch_metric_alarm" "backend_memory" {
  alarm_name          = "${local.name}-backend-memory-high"
  alarm_description   = "Backend ECS memory high"
  comparison_operator = "GreaterThanThreshold"
  evaluation_periods  = 2
  period              = 300
  namespace           = "AWS/ECS"
...
resource "aws_cloudwatch_metric_alarm" "frontend_memory" {
  alarm_name          = "${local.name}-frontend-memory-high"
  alarm_description   = "Frontend ECS memory high"
  comparison_operator = "GreaterThanThreshold"
  evaluation_periods  = 2
  period              = 300
  namespace           = "AWS/ECS"
...
resource "aws_cloudwatch_metric_alarm" "worker_cpu" {
  ...
  alarm_description   = "Worker ECS CPU high"
  comparison_operator = "GreaterThanThreshold"
  evaluation_periods  = 2
  period              = 300
  namespace           = "AWS/ECS"
...
resource "aws_cloudwatch_metric_alarm" "worker_memory" {
  ...
  alarm_description   = "Worker ECS memory high"
  comparison_operator = "GreaterThanThreshold"
  evaluation_periods  = 2
  period              = 300
  namespace           = "AWS/ECS"
...
resource "aws_cloudwatch_metric_alarm" "frontend_healthy_hosts" {
  alarm_name          = "${local.name}-frontend-healthy-hosts-low"
  alarm_description   = "Frontend target group has no healthy hosts"
...
resource "aws_cloudwatch_metric_alarm" "backend_healthy_hosts" {
  alarm_name          = "${local.name}-backend-healthy-hosts-low"
  alarm_description   = "Backend target group has no healthy hosts"
  comparison_operator = "LessThanThreshold"
  evaluation_periods  = 1
  period              = 60
  namespace           = "AWS/ApplicationELB"
